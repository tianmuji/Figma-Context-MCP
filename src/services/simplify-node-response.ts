...
import { isInAutoLayoutFlow, isFrame } from "~/utils/identity.js";
/**
 * TODO ITEMS
 *
...
/**
 * Determines if a node should include position information for layout inference
 */
function shouldIncludePositionInfo(
  ...
  if (isFrame(node) && node.layoutMode && node.layoutMode !== "NONE") {
    return false;
  }

  // 2. If node is in AutoLayout flow, it doesn't need position info
  if (parent && isInAutoLayoutFlow(node, parent)) {
    return false;
  }
  ...
  if (hasValue("layoutPositioning", node) && node.layoutPositioning === "ABSOLUTE") {
    return true;
  }
